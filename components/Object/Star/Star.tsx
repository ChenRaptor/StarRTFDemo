...
vec3 get_position(vec3 position)
{
    ...
    // Set up
    float value       = 0.0;
    float frequency_1 = 2.0;
    float speed_1     = 0.1;
    float frequency_2 = 24.0;
    float speed_2     = 0.2;

    // First global perlin (from 0 to 1)
    float perlin_1 = (cnoise(vec4(frequency_1 * position,uTime *time_multiplier * 0.6)) + 1.0) / 2.0;
    float perlin_2 = (cnoise(vec4(frequency_2 * position,uTime *time_multiplier * 0.8)) + 1.0) / 2.0;

    float value_1 = clamp(perlin_1,0.4,0.5) - 0.35;
    float value_2 = perlin_2;

    value = value_1 * value_2 * 0.50;
...
float get_intensity(vec3 position,float angle)
{
    ...
    //                            | frequency                                | speed
    float perlin_1 = (cnoise(vec4(2.0   * position,uTime * time_multiplier * 0.2)) + 1.0) / 2.0;
    float perlin_2 = (cnoise(vec4(6.0   * position,uTime * time_multiplier * 0.3)) + 1.0) / 2.0;
    float perlin_3 = (cnoise(vec4(12.0  * position,uTime * time_multiplier * 0.5)) + 1.0) / 2.0;
...
void main()
{
    ...
    {
        float displacement_1 = (cnoise(vec4(36.0 * new_position,uTime * 0.0003 * 0.5)) + 1.0) / 2.0;
...
const Star = ({ref}:any) => {
  ...
  useFrame(({ clock }) => {
    ...
    ref.current.material.uniforms.displacement.value = clock.getElapsedTime()/100;
    //meshRef.current.material.uniforms.ratio_step_1.value = clock.getElapsedTime()/10;
  });

  scene.fog = new THREE.Fog('#000000',300,60000)
    ...
    return (
        <mesh ref={ref} position={[0,0,0]}>
            ...
            uniforms={{
                ...
                color_step_1: { value: new THREE.Color(0xff0000) },
                color_step_2: { value: new THREE.Color(0xff6000) },
                color_step_3: { value: new THREE.Color(0xffff00) },
                color_step_4: { value: new THREE.Color(0xffffff) },

...
void main()
{
    ...
    // getting big region of noise by subtracting negative values
    float regionNoises = max(snoise(baseLayer), 0.0);

    // interpolate value of region of noises to smooth transition
    float smoothRegionNoises = mix(1.0, regionNoises, 0.7);

    // unify layers of noises and smoothed region
    ...
    vec3 smoothedLayersOfNoisesColored = convertBrightToColor((smoothedLayersOfNoises * 4.0 + 3.0)/3.);

    gl_FragColor = vec4(smoothedLayersOfNoisesColored, 1.0);
...
const Star = () => {
  ...
  useEffect(() => console.log(THREE.ShaderLib.phong.vertexShader),[])
                  ...
                  uniforms= {{
                    ...
                    uBrightnessAmplifier: {
                      value: THREE.MathUtils.randFloat(
                        sun.uBrightnessAmplifier.min,
                        sun.uBrightnessAmplifier.max
                      )
                    },
                    uNoiseIntensity: {
                      value: THREE.MathUtils.randFloat(
                        sun.uNoiseIntensity.min,
                        sun.uNoiseIntensity.max
                      )
                    },
                    uNoiseSpeed: {
                      value: THREE.MathUtils.randFloat(
                    ...
                    uColorAmplifierPrimary: { 
                      value: sun.uColorAmplifier.primary
                    },
                    uColorAmplifierSecondary: { 
                      value: sun.uColorAmplifier.secondary
                    },
                    uColorAmplifierTertiary: { 
                      value: sun.uColorAmplifier.tertiary
                    },
